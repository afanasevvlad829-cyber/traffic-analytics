...
ads_meta as (
    select
        ...
        max(coalesce(nullif(body_text, ''), '')) as body_text,
        max(coalesce(nullif(href, ''), '')) as href,
        max(coalesce(nullif(ad_type, ''), '')) as ad_type,
        max(coalesce(nullif(ad_subtype, ''), '')) as ad_subtype,
        max(coalesce(nullif(ad_image_hash, ''), '')) as ad_image_hash,
        max(creative_id) as creative_id,
        max(coalesce(nullif(thumbnail_url, ''), '')) as thumbnail_url,
        max(coalesce(nullif(preview_url, ''), '')) as preview_url
...
query_agg as (
    select
        ...
        case
            when sum(s.impressions) > 0 then round(sum(s.clicks)::numeric / sum(s.impressions), 4)
            else 0
        end as ctr,
        case
            when sum(s.clicks) > 0 then round(sum(s.cost) / sum(s.clicks), 2)
            else 0
        end as avg_cpc
    ...
    select
        ...
        avg(
            least(
                ...
                + 35 * (
                    case
                        when lower(coalesce(m.ad_title, '')) like '%' || lower(split_part(coalesce(d.search_query,''), ' ', 1)) || '%'
                        then 1 else 0 end
                )
                + 15 * (
                    case
                ...
                + 10 * (
                    case
                        when lower(coalesce(m.body_text, '')) like '%программ%' then 1 else 0 end
                )
                + 15 * (
                    case
                        when lower(coalesce(d.search_query, '')) like '%подмосков%' and
                             (lower(coalesce(m.ad_title,'')) like '%подмосков%' or lower(coalesce(m.body_text,'')) like '%подмосков%')
                        then 1 else 0 end
                )
            )
        ) as relevance_score
...
base as (
    select
        ...
        coalesce(m.ad_type, '') as ad_type,
        coalesce(m.ad_subtype, '') as ad_subtype,
        coalesce(m.ad_image_hash, '') as ad_image_hash,
        m.creative_id,
        coalesce(m.thumbnail_url, '') as thumbnail_url,
        coalesce(m.preview_url, '') as preview_url,
        q.impressions,
        q.clicks,
        q.cost,
        q.ctr,
        ...
        coalesce(b.avg_ctr, 0) as account_avg_ctr,
        coalesce(b.p50_ctr, 0) as account_median_ctr,
        coalesce(b.avg_cpc, 0) as account_avg_cpc,
        coalesce(b.p75_cpc, 0) as account_p75_cpc
    from query_agg q
    cross join bench b
    left join ads_meta m
...
scored as (
    select
        *,
        case
            when account_avg_ctr > 0
                then least(120, round(100 * ctr / account_avg_ctr, 2))
            else 50
        end as ctr_score,
        case
            when avg_cpc > 0 and account_avg_cpc > 0
                then least(120, round(100 * account_avg_cpc / avg_cpc, 2))
            else 50
        end as cpc_score,
        round(
            100 - least(
                ...
                case
                    when total_queries > 0
                        then (exclude_queries::numeric / total_queries) * 100
                    else 0
                end
            ),
            2
