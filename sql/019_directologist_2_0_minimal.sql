...
select
    ...
    decision,
    case
        when decision = 'EXCLUDE' then 'Запрос не похож на ваш продукт или относится к чужому лагерю/бренду.'
        when decision = 'WAIT_FOR_DATA' then 'Кампания слишком новая, данных ещё мало для уверенного решения.'
        when decision = 'LANDING_CHECK' then 'Интент коммерческий, CTR уже нормальный, но запрос ещё не дал результата.'
        when decision = 'REWRITE_AD' then 'CTR заметно ниже нормального уровня по аккаунту.'
        when decision = 'LOWER_BID_OR_SPLIT' then 'Цена клика выше верхнего нормального диапазона по аккаунту.'
        when decision = 'SCALE' then 'Запрос выглядит сильным относительно аккаунта.'
        else 'Данных пока недостаточно для сильного решения.'
    end as reason,
    case
        when decision = 'EXCLUDE'
            then concat('Исключить запрос/кластер: "', search_query, '". Добавить паттерн в минус-слова.')
        when decision = 'WAIT_FOR_DATA'
            then concat('Не трогать: кампания новая (', campaign_age_days, ' дн.).')
        when decision = 'LANDING_CHECK'
            then concat('Проверить посадочную под интент "', search_query, '". CTR=', round(ctr*100,2), '%, CPC=', avg_cpc, ' ₽.')
        when decision = 'REWRITE_AD'
            then concat('Переписать объявление: CTR=', round(ctr*100,2), '% против среднего ', round(avg_ctr*100,2), '%.')
        when decision = 'LOWER_BID_OR_SPLIT'
            then concat('Снизить ставку/вынести в отдельную группу: CPC=', avg_cpc, ' ₽, p75=', p75_cpc, ' ₽.')
        when decision = 'SCALE'
            then concat('Можно усиливать: CTR=', round(ctr*100,2), '%, CPC=', avg_cpc, ' ₽.')
        else concat('Наблюдать: показы=', impressions, ', клики=', clicks, '.')
    end as exact_action,
    case
...
from mart_direct_action_queue_v3
group by date, campaign_name, decision;
