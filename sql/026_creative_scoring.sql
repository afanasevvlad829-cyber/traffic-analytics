drop view if exists vw_creative_score_report;
drop table if exists mart_direct_creative_score;
...
ads_meta as (
    select
        ...
        max(nullif(title, '')) as ad_title,
        max(nullif(title2, '')) as ad_title_2,
        max(nullif(text_preview, '')) as ad_text,
        max(nullif(href, '')) as href
...
scored as (
    select
        *,
        least(120, round(100 * ctr / nullif(account_avg_ctr, 0), 2)) as ctr_score,
